(function() {
  Discourse.ShareLink.addTarget('weibo', {
    ...
    generateUrl: function(link, title) {
      return ("http://service.weibo.com/share/share.php?url=" + encodeURIComponent(link) + "&title=苹果吧Pgbar - " + encodeURIComponent(title))+ "&searchPic=true&style=number";
    },
  ...
  Discourse.ShareLink.addTarget('renren', {
    iconClass: 'fa-renren',
    generateUrl: function(link, title) {
      return ("http://widget.renren.com/dialog/share?resourceUrl=" + encodeURIComponent(link) + "&title=苹果吧Pgbar - " + encodeURIComponent(title) + "&description=" + encodeURIComponent(title));
    },
    shouldOpenInPopup: true,
    popupHeight: 628
  });
